<template>
  <div class="carte-resultats-section">
    <v-text-field v-model="toget.election" label="Élection"/>
    <v-text-field v-model="toget.circonscription" label="Circonscription"/>
...
export default {
    ...
    sectionsMap() {
      ...
      if (sectionsList.length > 0) {
        ...
        for (var section of sectionsList) {
          ...
          let polygone = _this.readGeoJSON(section.polygone.polygone)
  ...
  methods: {
    get() {
      ...
      axios.get(`/api/circonscriptions/${this.toget.circonscription}?relations=sections.sectionresultats.parti,sections.sectionresultats.personne,sections.sectionparticipation,sections.municipalite,sections.sectionpolygone&election=${this.toget.election}&scrutin=1`)
      .then(response => {
        this.sections.list.push(...response.data.data.sections)
