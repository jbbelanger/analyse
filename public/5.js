...
/* harmony default export */ __webpack_exports__["default"] = ({
      ...
      if (sectionsList.length > 0) {
        ...
        try {
          for (var _iterator = sectionsList[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            ...
            var polygone = _this.readGeoJSON(section.polygone, section.id); //vérifier si la section contient des résultats et un polygone
  ...
  methods: {
    get: function get() {
      ...
      axios.get("/api/circonscriptions/".concat(this.$route.params.circonscriptionId, "?relations=sections.sectionresultats.parti,sections.sectionresultats.personne,sections.sectionparticipation,sections.municipalite,sections.sectionpolygone&election=").concat(this.$route.params.electionId, "&scrutin=1")).then(function (response) {
        _this4.sections.list = response.data.data.sections;
    ...
    readGeoJSON: function readGeoJSON(geojson, id) {
      if (geojson) {
        var g = JSON.parse(geojson.polygone);
